...
function sortQuestionsBySets(questions, sets) {
  if (!sets?.length) return questions
  const grouped = new Map(sets.map((set) => [Number(set.set_id), []]))
  const unassigned = []
  for (const question of questions) {
    const setId = question.setId != null ? Number(question.setId) : null
    if (setId && grouped.has(setId)) grouped.get(setId).push(question)
    else unassigned.push(question)
  }
  return [...sets.flatMap((set) => grouped.get(Number(set.set_id)) || []), ...unassigned]
}
...
function BuilderPage() {
  ...
  const commitSetRename = (set) => {
    const trimmed = setRenameDraft.trim()
    setRenamingSetId(null)
    if (!trimmed || trimmed === set.name) return
    renameSetMutation.mutate({ setId: set.set_id, name: trimmed })
  }

  /**
   * Question list grouped for display: one group per set (in set order) plus a trailing
   * "not in a set" group. Flattened, this matches the order used on drag and on save.
   */
  const questionGroups = useMemo(() => {
    if (!questionSets.length) return [{ set: null, questions }]
    const bySet = new Map(questionSets.map((set) => [Number(set.set_id), []]))
    const unassigned = []
    for (const question of questions) {
      const setId = question.setId != null ? Number(question.setId) : null
      if (setId && bySet.has(setId)) bySet.get(setId).push(question)
      else unassigned.push(question)
    }
    return [
      ...questionSets.map((set) => ({ set, questions: bySet.get(Number(set.set_id)) })),
      { set: null, questions: unassigned },
    ]
  }, [questions, questionSets])

  const visualQuestions = useMemo(
    () => questionGroups.flatMap((group) => group.questions),
    [questionGroups],
  )

  const renderSetGroupHeader = (group) => {
    if (!questionSets.length) return null
    if (!group.set) {
      return (
        <div className="flex items-center gap-2 rounded-xl border border-amber-200/80 bg-amber-50/70 px-2.5 py-1.5">
          <p className="min-w-0 flex-1 truncate text-xs font-bold text-amber-900">
            Not in a set
            <span className="ml-1.5 font-medium text-amber-800/80">
              · shown to all participants
            </span>
          </p>
          <span className="shrink-0 text-[11px] font-semibold text-amber-800/80">
    ...
    const set = group.set
    const isActive = Number(activeSetId) === Number(set.set_id)
    const isRenaming = Number(renamingSetId) === Number(set.set_id)
    return (
      <div
        className={`flex items-center gap-1.5 rounded-xl border px-2.5 py-1.5 ${
          isActive && canManageSets
      ...
      >
        {isRenaming ? (
          <input
            ...
            maxLength={80}
            onChange={(event) => setSetRenameDraft(event.target.value)}
            onBlur={() => commitSetRename(set)}
            onKeyDown={(event) => {
              if (event.key === 'Enter') commitSetRename(set)
              if (event.key === 'Escape') setRenamingSetId(null)
            }}
            className="h-7 min-w-0 flex-1 rounded-lg border border-indigo-300 bg-white px-2 text-xs font-semibold text-indigo-950 outline-none"
          />
        ) : (
          <button
            type="button"
            onClick={() => (canManageSets ? setActiveSetId(Number(set.set_id)) : undefined)}
            title={
              canManageSets
                ? 'Select this set — new questions are added to the selected set'
                : undefined
            }
            className="min-w-0 flex-1 truncate text-left text-xs font-bold text-indigo-950"
          >
            {set.name}
            <span className="ml-1.5 font-medium text-indigo-700/70">
        ...
        {canManageSets && !isRenaming ? (
          <>
            {canQuickAddQuestion ? (
              <button
                type="button"
                title={`Add a ${sessionQuestionType} question to ${set.name}`}
                onClick={() => {
                  setActiveSetId(Number(set.set_id))
                  addQuestion(sessionQuestionType, Number(set.set_id))
                }}
                className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-indigo-200 bg-white px-2 py-1 text-[11px] font-semibold text-indigo-900 transition hover:bg-indigo-100"
              >
            ...
            <button
              ...
              title="Rename set"
              onClick={() => {
                setRenamingSetId(set.set_id)
                setSetRenameDraft(set.name)
              }}
            ...
            <button
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    `Delete ${set.name}? Its questions stay in the session, outside any set.`,
                  )
                ) {
                  return
                }
                deleteSetMutation.mutate(set.set_id)
              }}
  ...
  const renameSetMutation = useMutation({
    mutationFn: ({ setId, name }) =>
      updateQuestionSetApi(accessToken, Number(sessionId), setId, name),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['builder-question-sets', sessionId] })
    },
    onError: (error) => setSaveError(error.message || 'Unable to rename set'),
  })

  const deleteSetMutation = useMutation({
    mutationFn: (setId) => deleteQuestionSetApi(accessToken, Number(sessionId), setId),
    onSuccess: (_data, setId) => {
      setQuestions((prev) =>
        prev.map((question) =>
          Number(question.setId) === Number(setId) ? { ...question, setId: null } : question,
        ),
      )
      setDirty(true)
      if (Number(activeSetId) === Number(setId)) setActiveSetId(null)
      queryClient.invalidateQueries({ queryKey: ['builder-question-sets', sessionId] })
    },
    onError: (error) => setSaveError(error.message || 'Unable to delete set'),
  })
  ...
  useEffect(() => {
    ...
    if (!activeSetId || !questionSets.some((set) => Number(set.set_id) === Number(activeSetId))) {
      setActiveSetId(Number(questionSets[0].set_id))
    }
  ...
  const onQuestionsDragEnd = (event) => {
    ...
    const { active, over } = event
    if (!over || active.id === over.id) return
    const visual = sortQuestionsBySets(questions, questionSets)
    const oldIndex = visual.findIndex((q) => q.id === active.id)
    const newIndex = visual.findIndex((q) => q.id === over.id)
    if (oldIndex < 0 || newIndex < 0) return
    const overQuestion = visual[newIndex]
    const reordered = arrayMove(visual, oldIndex, newIndex).map((q) =>
      q.id === active.id ? { ...q, setId: overQuestion.setId ?? null } : q,
    )
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const questionsToSave = sortQuestionsBySets(questions, questionSets)
      const orderedIds = []
      for (let index = 0; index < questionsToSave.length; index += 1) {
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['builder-question-sets', sessionId] })
          ...
          <div className="rounded-2xl border border-blue-200/70 bg-white/70 p-5 shadow-sm shadow-blue-900/5 backdrop-blur">
            ...
            <div className="mt-4 max-h-[calc(100vh-270px)] space-y-2 overflow-auto pr-1">
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onQuestionsDragEnd}>
                <SortableContext items={visualQuestions.map((q) => q.id)} strategy={verticalListSortingStrategy}>
                  <div className="space-y-3">
                    {questionGroups.map((group) => {
                      if (!group.set && questionSets.length > 0 && !group.questions.length) {
                        return null
                      }
                      return (
                        <div
                          key={group.set ? `set-${group.set.set_id}` : 'no-set'}
                          className="space-y-2"
                        >
                      ...
                      <div key={q.id} className={`rounded-2xl ${q.id === selectedId ? 'ring-2 ring-blue-500/25' : ''}`}>
                        <SortableRow id={q.id} className="border-blue-200/70">
                          ...
                          {group.set && canManageSets && !group.questions.length ? (
                            <p className="rounded-xl border border-dashed border-indigo-200 bg-white/60 px-3 py-2 text-[11px] leading-relaxed text-slate-500">
                              No questions in this set yet — press <strong>Add</strong> on the
                              heading above, or move a question here from the editor.
                            </p>
                          ) : null}
                        </div>
                      )
                        ...
                        setQuestions((prev) =>
                          prev.map((question) =>
                            question.id === selected.id
                              ? { ...question, setId: value ? Number(value) : null }
                              : question,
                          ),
                        )
                      }}
