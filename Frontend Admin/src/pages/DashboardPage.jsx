...
function DashboardPage() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error, variables) => {
      const title = variables?.input?.title
      const detailList = Array.isArray(error?.details)
        ? error.details.filter(Boolean)
        : []
      const detailText = detailList.length ? ` ${detailList.join('; ')}` : ''
      const reason = `${error.message || 'Please try again.'}${detailText}`
      setSessionAlert({
        variant: 'error',
        title: 'Could not update session',
        message: title ? `Failed to save "${title}". ${reason}` : reason,
        confirmLabel: 'Close',
      })
    },
  })
  ...
  const editSessionInitial = useMemo(() => {
    ...
    return {
      ...
      showParticipantCount: Boolean(editSession.show_participant_count),
