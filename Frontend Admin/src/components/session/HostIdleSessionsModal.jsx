...
export function HostIdleSessionsModal({
  ...
  const confirmSession = useMemo(
    () => sessions.find((session) => sessionKey(session) === String(confirmEndId)) || null,
    [sessions, confirmEndId],
  )

  useEffect(() => {
    if (!open) setConfirmEndId(null)
  }, [open])

  useEffect(() => {
    if (!confirmEndId) return
    if (outcomes[confirmEndId] === 'ended') {
      setConfirmEndId(null)
    }
  }, [confirmEndId, outcomes])

  useEffect(() => {
    if (!open || confirmEndId) return undefined
    const onKeyDown = (event) => {
      if (event.key === 'Escape' && allResolved) onClose?.()
    }
    document.addEventListener('keydown', onKeyDown)
    return () => document.removeEventListener('keydown', onKeyDown)
  }, [open, allResolved, confirmEndId, onClose])
  ...
  return (
    <>
              ...
              {sessions.map((session) => {
                ...
                return (
                  <IdleSessionRow
                    ...
                    disabled={Boolean(pendingId) || Boolean(confirmEndId)}
                    onStay={() => onStay(id)}
                    onRequestEnd={() => setConfirmEndId(id)}
                  />
                )
              })}
            </div>
      ...
      <EndSessionConfirmModal
        ...
        sessionTitle={confirmSession?.title}
        endPending={pendingId === confirmEndId && pendingAction === 'end'}
