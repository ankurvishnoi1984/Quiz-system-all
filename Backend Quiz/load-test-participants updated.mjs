 ...
 *
 * Usage (bash):
 *   API_BASE_URL=https://demoquizapi.netcastservice.online/api/v1 SESSION_CODE=CEW2M4 USER_COUNT=52 node Backend\ Quiz/load-test-participants\ updated.mjs
 *
 * Env:
...
function loadWebSocketImpl() {
  ...
  try {
    return require(join(__dirname, '/node_modules/ws'))
  } catch {
    throw new Error(
  ...
  async function runner() {
    while (true) {
      ...
      try {
        ...
        if (
          err.status === 403 ||
          /participant limit|plan limit|limit exceeded/i.test(msg)
        ) {
          results.limitRejected += 1
        }
        if (results.errors.length < 12) {
          results.errors.push(`User ${index}: ${msg}`)
        }
        onProgress?.(results, total)
      }
    }
...
async function main() {
  ...
  console.log('')
  console.log(
    'Note: plan limits count live WebSocket connections. Keep WS_ENABLED=true when testing plan caps.',
  )
  console.log(
    'Tip: set USER_COUNT above the plan limit and CONCURRENCY high to verify the cap under load.',
  )
  console.log('')
  ...
  if (session.plan_limit != null || session.participant_limit != null) {
    console.log(
      `Limit hint: ${session.plan_limit ?? session.participant_limit ?? 'n/a'} (from join lookup if present)`,
    )
  }
  console.log('')
  ...
  console.log(`Limit rejected:   ${results.limitRejected}`)
  if (WS_ENABLED) {
    console.log(`WS connected:     ${results.wsOk}`)
    console.log(`WS failed:        ${results.wsFail}`)
    console.log(`WS held at end:   check Host UI / live count (should be ≤ plan limit)`)
  }
  if (SUBMIT_ANSWERS && !JOIN_ONLY) {
  ...
  // Expected when USER_COUNT > plan limit: some failures are limit rejects.
  const unexpectedFails = results.fail - results.limitRejected
  if (unexpectedFails > 0) process.exit(1)
}
